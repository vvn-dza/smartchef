...
async function generateRecommendationsAndNotify() {
  ...
  for (const userDoc of usersSnap.docs) {
    ...
    // Send daily notification with top recommendation
    if (sorted.length > 0) {
      const [topRecipeId, topScore] = sorted[0];
      const notificationsCol = db
        .collection('users')
        .doc(userId)
        .collection('notifications');
      const message = `Today's pick for you! Check out recipe ${topRecipeId} (score: ${topScore}).`;
      await notificationsCol.add({
        message,
        recipeId: topRecipeId,
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
      });
      console.log(`Notification sent to user ${userId} for recipe ${topRecipeId}`);
    }
    console.log(`Updated recommendations for user ${userId}`);
  }
  console.log('All user recommendations and notifications updated.');
}

if (require.main === module) {
  generateRecommendationsAndNotify()
    .then(() => process.exit(0))
    .catch((err) => {
      console.error(err);
      process.exit(1);
